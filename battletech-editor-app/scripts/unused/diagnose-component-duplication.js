...
const { UnitCriticalManager } = require('./utils/criticalSlots/UnitCriticalManager.ts')
...
function countSpecialComponents(unit, componentType) {
...
function testComponentDuplication() {
  ...
  try {
    ...
    const newConfig1 = { ...unit.getConfiguration(), structureType: 'Standard' }
    ...
    const newConfig2 = { ...unit.getConfiguration(), armorType: 'Ferro-Fibrous' }
    ...
    const newConfig3 = { ...unit.getConfiguration(), structureType: 'Endo Steel' }
    ...
    for (let i = 0; i < 5; i++) {
      const configA = { ...unit.getConfiguration(), structureType: 'Standard' }
      unit.updateConfiguration(configA)
      const configB = { ...unit.getConfiguration(), structureType: 'Endo Steel' }
    ...
    console.log('Unit internal state:', {
      specialComponentsInitialized: unit.specialComponentsInitialized,
