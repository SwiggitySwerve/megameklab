/**
 * Updated Criticals Tab Component 
 * Uses the object-based critical slot system with full drag & drop support
 */
...
interface CriticalsTabUpdatedProps {
  readOnly?: boolean;
}

// Extended equipment interface to track the original index
interface TrackedFullEquipment extends FullEquipment {
...
export default function CriticalsTabUpdated({ readOnly = false }: CriticalsTabUpdatedProps) {
  ...
  // Map to track equipment index by a stable key
  const [equipmentIndexMap, setEquipmentIndexMap] = useState<Map<string, number>>(new Map());
  ...
  // Update equipment index map whenever equipment changes
  useEffect(() => {
    const indexMap = new Map<string, number>();
    equipment.forEach((eq, index) => {
      // Create a unique key for each equipment item
      const key = `${eq.item_name}-${index}-${eq.location || 'unallocated'}`;
      indexMap.set(key, index);
    });
    setEquipmentIndexMap(indexMap);
  }, [equipment]);
  
  // Track if we've initialized from critical allocations
  const [isInitialized, setIsInitialized] = useState(false);
  
  // Initialize slots from critical allocations ONLY ONCE
  useEffect(() => {
    if (criticalAllocations && !isInitialized) {
      ...
      Object.entries(criticalAllocations).forEach(([location, locationSlots]) => {
        ...
        newSlots[location] = Array(mechLocation.slots).fill(null).map((_, index) => {
          ...
          // Handle different slot formats - using the actual structure from useUnitData
          ...
          // Find actual equipment data - use current equipment state
          const currentEquipment = equipment;
          const equipmentItem = currentEquipment.find(eq => eq.item_name === normalizedName && eq.location === location);
          ...
          const equipmentData: EquipmentObject = {
            ...
            isFixed: slot.isFixed || isFixedComponent(normalizedName),
            isRemovable: !slot.isFixed && !isFixedComponent(normalizedName),
    ...
    // Remove equipment from dependencies - we only want to initialize once
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [criticalAllocations, isInitialized, state.unit.tech_base]);
  
  // Get unallocated equipment
  const unallocatedEquipment = useMemo(() => {
    const unallocated: TrackedFullEquipment[] = [];
    
    equipment.forEach((eq, index) => {
      // Skip if equipment has a location
      if (eq.location && eq.location !== '') return;
  ...
  const handleDrop = (equipment: EquipmentObject, location: string, slotIndex: number) => {
    ...
    const slotsForUpdate = newSlots[location].map(slot => 
      slot.equipment ? slot.equipment.equipmentData.name : '-Empty-'
    );
    updateCriticalSlots(location, slotsForUpdate);
    
    // Find the equipment index from the tracked data
    const trackedEquipment = unallocatedEquipment.find(eq => eq.id === equipment.id) as TrackedFullEquipment;
    if (trackedEquipment && trackedEquipment.originalIndex !== undefined) {
      updateEquipmentLocation(trackedEquipment.originalIndex, location);
    } else {
      // Fallback: try to find by name
      const equipmentIndex = state.unit.data?.weapons_and_equipment?.findIndex(eq => 
        eq.item_name === equipment.name && (!eq.location || eq.location === '')
      );
  ...
  const handleRemove = (location: string, slotIndex: number) => {
    ...
    const newSlots = { ...criticalSlots };
    let firstSlotIndex = slotIndex;
    
    if (slot.isPartOfMultiSlot && slot.multiSlotGroupId) {
      // Find the first slot of this multi-slot equipment
      firstSlotIndex = slot.equipment.startSlotIndex;
    ...
    // Clear equipment location - find the specific instance
    const equipmentIndices: number[] = [];
    state.unit.data?.weapons_and_equipment?.forEach((eq, index) => {
      if (eq.item_name === equipment.name && eq.location === location) {
        equipmentIndices.push(index);
      }
    });
    
    // For multi-slot equipment, clear the first matching equipment
    // For single-slot, we need to be more careful about which instance to clear
    if (equipmentIndices.length > 0) {
      // Clear the first matching equipment at this location
      updateEquipmentLocation(equipmentIndices[0], '');
    }
  };
  ...
  const handleMove = (fromLocation: string, fromIndex: number, toLocation: string, toIndex: number) => {
    ...
    // Find which equipment instance we're moving
    let equipmentInstanceIndex = -1;
    state.unit.data?.weapons_and_equipment?.forEach((eq, index) => {
      if (eq.item_name === equipment.name && eq.location === fromLocation && equipmentInstanceIndex === -1) {
        equipmentInstanceIndex = index;
      }
    });
    
    if (equipmentInstanceIndex === -1) return;
    ...
    // Update the unit data for both locations
    const sourceSlotsForUpdate = newSlots[fromLocation].map(slot => 
      slot.equipment ? slot.equipment.equipmentData.name : '-Empty-'
    );
    updateCriticalSlots(fromLocation, sourceSlotsForUpdate);
    
    if (fromLocation !== toLocation) {
      const targetSlotsForUpdate = newSlots[toLocation].map(slot => 
        slot.equipment ? slot.equipment.equipmentData.name : '-Empty-'
      );
      updateCriticalSlots(toLocation, targetSlotsForUpdate);
      
      // Update equipment location
      updateEquipmentLocation(equipmentInstanceIndex, toLocation);
    }
  };
  ...
  const clearLocation = (location: string) => {
    ...
    const newSlots = { ...criticalSlots };
    const clearedEquipment: Map<string, number> = new Map();
    
    // Track equipment to clear by name and count
    newSlots[location].forEach((slot, index) => {
      if (slot.equipment && !slot.equipment.equipmentData.isFixed) {
        const name = slot.equipment.equipmentData.name;
        clearedEquipment.set(name, (clearedEquipment.get(name) || 0) + 1);
    ...
    // Clear equipment locations - only clear the number of instances we removed
    clearedEquipment.forEach((count, equipmentName) => {
      let cleared = 0;
      state.unit.data?.weapons_and_equipment?.forEach((eq, index) => {
        if (eq.location === location && eq.item_name === equipmentName && cleared < count) {
