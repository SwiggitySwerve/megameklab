...
interface CriticalSlotDropZoneV2Props {
  location: string;
  slotIndex: number;
...
const CriticalSlotDropZoneV2: React.FC<CriticalSlotDropZoneV2Props> = ({
  location,
  slotIndex,
  slot,
  onDrop,
  ...
  const [{ isOver, canDrop }, drag] = useDrop({
    ...
    canDrop: (item: DraggedEquipmentV2) => {
      if (disabled || hasEquipment) return false;
    ...
    hover: (item: DraggedEquipmentV2, monitor) => {
      if (onHoverChange && monitor.isOver({ shallow: true })) {
        const equipmentObj: EquipmentObject = {
          id: item.equipmentId,
          name: item.name,
          type: item.type as any,
...
export default CriticalSlotDropZoneV2;
