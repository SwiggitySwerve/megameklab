...
export const ARMOR_SLOT_REQUIREMENTS = {
    'Standard': { slots: 0 },
...
export const ARMOR_TECH_RESTRICTIONS = {
    'Standard': {
...
export const ARMOR_SPECIAL_PROPERTIES = {
    'Standard': {},
    'Ferro-Fibrous': {},
    'Ferro-Fibrous (Clan)': {},
    'Light Ferro-Fibrous': {},
 ...
 */
export function calculateArmorWeight(armorPoints, type) {
 ...
 */
export function getArmorSlots(type, techBase) {
 ...
 */
export function calculateMaxArmorForTonnage(tonnage, type) {
    const pointsPerTon = ARMOR_POINTS_PER_TON[type];
 ...
 */
export function distributeArmorPoints(totalPoints, distribution) {
    const result = {};
 ...
 */
export function validateArmorType(type, techBase, rulesLevel, hasRequiredEquipment) {
    ...
    if (restrictions.additionalRequirements && restrictions.additionalRequirements.length > 0) {
        meetsAdditionalReqs = restrictions.additionalRequirements.every(req => hasRequiredEquipment?.includes(req) || false);
    }
 ...
 */
export function getArmorSpecialProperties(type) {
    return ARMOR_SPECIAL_PROPERTIES[type] || {};
}
/**
 * Calculate armor coverage percentage
 */
export function calculateArmorCoverage(currentArmorPoints, maxArmorPoints) {
    if (maxArmorPoints === 0)
        return 0;
 ...
 */
export function getArmorCalculations(armorPoints, type, techBase) {
    return {
