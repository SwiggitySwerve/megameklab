 ...
 */
export const JUMP_JET_VARIANTS = {
    'Standard Jump Jet': {
 ...
 */
export function calculateJumpJetWeight(jumpJetType, mechTonnage, jumpMP = 1) {
 ...
 */
export function calculateJumpJetCriticalSlots(jumpJetType, mechTonnage) {
    const variant = JUMP_JET_VARIANTS[jumpJetType];
 ...
 */
export function getMaxAllowedJumpMP(jumpJetType, walkMP, runMP) {
    const variant = JUMP_JET_VARIANTS[jumpJetType];
 ...
 */
export function calculateTotalJumpJetWeight(jumpJetCounts, mechTonnage, hasPartialWing = false) {
    let totalWeight = 0;
 ...
 */
export function calculateTotalJumpJetCrits(jumpJetCounts, mechTonnage) {
    let totalCrits = 0;
    Object.entries(jumpJetCounts).forEach(([type, count]) => {
        if (!count || count <= 0)
            return;
 ...
 */
export function validateJumpJetConfiguration(jumpJetCounts, requestedJumpMP, walkMP, runMP, mechTonnage) {
    const errors = [];
    const warnings = [];
 ...
 */
export function calculateJumpJetHeat(jumpJetCounts, jumpMP) {
    let totalHeat = 0;
    Object.entries(jumpJetCounts).forEach(([type, count]) => {
        if (!count || count <= 0)
            return;
 ...
 */
export function calculateJumpJetCost(jumpJetCounts) {
    let totalCost = 0;
    Object.entries(jumpJetCounts).forEach(([type, count]) => {
        if (!count || count <= 0)
            return;
 ...
 */
export function getAvailableJumpJetTypes(techBase, rulesLevel = 'Standard') {
