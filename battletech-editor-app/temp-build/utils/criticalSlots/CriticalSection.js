...
export class CriticalSection {
    constructor(location, configuration) {
     ...
     */
    findContiguousAvailableSlots(requiredSlots) {
     ...
     */
    findConflictingEquipment(reservedSlots) {
        const conflictingEquipment = new Map();
     ...
     */
    reserveSystemSlots(componentType, slots) {
     ...
     */
    clearSystemReservations(componentType) {
     ...
     */
    allocateEquipment(equipment, startSlot, equipmentGroupId = uuidv4()) {
     ...
     */
    removeEquipmentGroup(equipmentGroupId) {
     ...
     */
    canMoveEquipmentToSlots(equipmentGroupId, newStartSlot) {
        const equipment = this.equipmentRegistry.get(equipmentGroupId);
        if (!equipment)
            return false;
        ...
        // Check bounds
        if (newSlots[newSlots.length - 1] >= this.slots.length)
            return false;
     ...
     */
    moveEquipmentToSlots(equipmentGroupId, newStartSlot) {
        ...
        this.equipmentRegistry.forEach((allocation, groupId) => {
            allocation.occupiedSlots.forEach(slotIndex => {
                ...
                if (slot.content?.equipmentGroupId !== groupId) {
                    result.isValid = false;
                    result.errors.push(`Equipment registry mismatch for group ${groupId} at slot ${slotIndex + 1}`);
                }
            });
        });
        return result;
