...
export class UnitConfigurationBuilder {
     ...
     */
    static buildConfiguration(input) {
        // Handle legacy configuration
        if ('mass' in input && !('tonnage' in input)) {
            return this.fromLegacyConfiguration(input);
        }
     ...
     */
    static fromLegacyConfiguration(legacy) {
     ...
     */
    static calculateDependentValues(config) {
     ...
     */
    static calculateInternalHeatSinks(engineRating, engineType) {
     ...
     */
    static validateEngineRating(tonnage, walkMP) {
...
export class UnitCriticalManager {
    ...
    getArmorCriticalSlots(armorType) {
        try {
            return getArmorSlots(armorType, this.configuration.techBase) || 0;
        }
        catch (error) {
     ...
     */
    getStructureCriticalSlots(structureType) {
        const structureSlotMap = {
            'Standard': 0,
            'Endo Steel': 14,
            'Endo Steel (Clan)': 7,
            'Composite': 0,
    ...
    allocateSystemComponents() {
        const systemAllocation = SystemComponentRules.getCompleteSystemAllocation(this.configuration.engineType, this.configuration.gyroType);
     ...
     */
    allocateEngineSlots(engineAllocation) {
     ...
     */
    allocateGyroSlots(gyroAllocation) {
     ...
     */
    handleSystemComponentChange(oldConfig, newConfig) {
        const allDisplacedEquipment = [];
        ...
        const displacementImpact = SystemComponentRules.getDisplacementImpact(oldConfig.engineType, oldConfig.gyroType, newConfig.engineType, newConfig.gyroType);
     ...
     */
    allocateSystemComponentsWithConfig(config) {
        const systemAllocation = SystemComponentRules.getCompleteSystemAllocation(config.engineType, config.gyroType);
        // Allocate engine slots
        this.allocateEngineSlots(systemAllocation.engine);
        // Allocate gyro slots
        this.allocateGyroSlots(systemAllocation.gyro);
     ...
     */
    handleSpecialComponentConfigurationChange(oldConfig, newConfig) {
        // Handle structure type changes
        if (oldConfig.structureType !== newConfig.structureType) {
            this.updateSpecialComponents(oldConfig.structureType, newConfig.structureType, 'structure');
        }
        // Handle armor type changes
        if (oldConfig.armorType !== newConfig.armorType) {
            this.updateSpecialComponents(oldConfig.armorType, newConfig.armorType, 'armor');
        }
     ...
     */
    updateSpecialComponents(oldType, newType, componentType) {
        // Remove old special components if they exist
        const oldSlots = componentType === 'armor'
            ? this.getArmorCriticalSlots(oldType)
            : this.getStructureCriticalSlots(oldType);
        if (oldSlots > 0) {
     ...
     */
    addSpecialComponents(type, componentType, requiredSlots) {
     ...
     */
    createSpecialComponentEquipment(type, componentType, requiredSlots) {
     ...
     */
    updateJumpJetEquipment(oldConfig, newConfig) {
     ...
     */
    removeJumpJetEquipment() {
        // Remove from unallocated equipment
        this.unallocatedEquipment = this.unallocatedEquipment.filter(eq => !eq.equipmentData.name.includes('Jump') &&
            !eq.equipmentData.name.includes('UMU') &&
            !eq.equipmentData.name.includes('Booster') &&
            !eq.equipmentData.name.includes('Wing'));
        // Remove from critical slots across all sections
        this.sections.forEach(section => {
            const equipmentToRemove = section.getAllEquipment().filter(eq => eq.equipmentData.name.includes('Jump') ||
                eq.equipmentData.name.includes('UMU') ||
                eq.equipmentData.name.includes('Booster') ||
                eq.equipmentData.name.includes('Wing'));
            equipmentToRemove.forEach(eq => {
     ...
     */
    addJumpJetEquipment(jumpJetType, jumpMP, tonnage, techBase) {
     ...
     */
    findEquipmentGroup(equipmentGroupId) {
        ...
        const unallocated = this.unallocatedEquipment.find((eq) => eq.equipmentGroupId === equipmentGroupId);
        if (unallocated) {
     ...
     */
    addUnallocatedEquipment(equipment) {
     ...
     */
    removeUnallocatedEquipment(equipmentGroupId) {
     ...
     */
    displaceEquipment(equipmentGroupId) {
     ...
     */
    canPlaceEquipmentInLocation(equipment, location) {
     ...
     */
    hasEngineSlots(location) {
        const section = this.getSection(location);
        if (!section)
            return false;
        // Check if this location has engine slots based on current engine configuration
        const engineAllocation = SystemComponentRules.getCompleteSystemAllocation(this.configuration.engineType, this.configuration.gyroType);
        switch (location) {
            case 'Center Torso':
     ...
     */
    getLocationRestrictionError(equipment, location) {
        if (equipment.allowedLocations) {
     ...
     */
    allocateEquipmentFromPool(equipmentGroupId, location, startSlot) {
    ...
    validate() {
        ...
        const systemValidation = SystemComponentRules.validateSystemComponents(this.configuration.engineType, this.configuration.gyroType);
