...
export class SystemComponentRules {
     ...
     */
    static getGyroAllocation(gyroType) {
        const allocation = {
            centerTorso: [],
            leftTorso: [],
            rightTorso: []
     ...
     */
    static getEngineAllocation(engineType, gyroType) {
        const allocation = {
     ...
     */
    static getCompleteSystemAllocation(engineType, gyroType) {
     ...
     */
    static validateSystemComponents(engineType, gyroType) {
        const result = {
            isValid: true,
            errors: [],
            warnings: []
        ...
        try {
            ...
            if (maxCenterTorsoSlot >= 12) {
                ...
                result.errors.push(`${engineType} engine with ${gyroType} gyro requires slot ${maxCenterTorsoSlot + 1}, but Center Torso only has 12 slots`);
            }
     ...
     */
    static getAvailableEquipmentSlots(engineType, gyroType) {
        const allocation = this.getCompleteSystemAllocation(engineType, gyroType);
        ...
        Object.entries(locationSlots).forEach(([location, totalSlots]) => {
            const systemSlots = allocation.combined[location] || [];
            const fixed = fixedSlots[location] || [];
     ...
     */
    static getMaxEquipmentSizes(engineType, gyroType) {
     ...
     */
    static canEquipmentFit(equipmentSlots, location, engineType, gyroType) {
     ...
     */
    static getDisplacementImpact(oldEngineType, oldGyroType, newEngineType, newGyroType) {
        ...
        const locations = ['centerTorso', 'leftTorso', 'rightTorso'];
     ...
     */
    static getSystemDescription(engineType, gyroType) {
        const allocation = this.getCompleteSystemAllocation(engineType, gyroType);
        const descriptions = [];
        // Engine description
        const engineSlots = allocation.engine;
        if (engineSlots.centerTorso.length > 3) {
            descriptions.push(`${engineType} Engine: CT slots ${engineSlots.centerTorso.map(s => s + 1).join(', ')}`);
        }
        else {
        ...
        // Gyro description
        descriptions.push(`${gyroType} Gyro: CT slots ${allocation.gyro.centerTorso.map(s => s + 1).join(', ')}`);
