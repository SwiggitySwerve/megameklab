...
// Test functions
function testStandardComponents(): boolean {
...
function testEndoSteelComponents(): boolean {
  ...
  const endoSteelComponents = unallocated.filter(eq => 
    ...
    (eq.equipmentData as any).componentType === 'structure'
  );
...
function testFerroFibrousComponents(): boolean {
...
function testCombinedComponents(): boolean {
...
function testConfigurationChange(): boolean {
  ...
  const newConfig = { ...config, structureType: 'Endo Steel' as const };
...
function testJumpJetComponents(): boolean {
