...
interface MTFData {
  ...
  // Armor
  armor: string;
  armor_values: { [location: string]: number | { front: number; rear: number } };
  
  // Weapons & Equipment
  weapons: Array<{ name: string; location: string; ammo?: number }>;
  ...
  systemmanufacturer?: { [system: string]: string };
...
export function exportToMTF(unit: EditableUnit): string {
  ...
  // Quirks
  if (unit.data?.quirks && unit.data.quirks.length > 0) {
    lines.push('Quirks:');
    unit.data.quirks.forEach((quirk: any) => {
      lines.push(quirk.name || quirk);
    });
    lines.push('');
  }
  
  // Fluff
  if (unit.data?.history) {
    lines.push('History:');
    lines.push(unit.data.history);
    lines.push('');
  }
  
  if (unit.data?.overview) {
    lines.push('Overview:');
    lines.push(unit.data.overview);
    lines.push('');
  }
  
  if (unit.data?.capabilities) {
    lines.push('Capabilities:');
    lines.push(unit.data.capabilities);
    lines.push('');
  }
  
  if (unit.data?.deployment) {
    lines.push('Deployment:');
    lines.push(unit.data.deployment);
    lines.push('');
  }
  
  // Manufacturer info
  if (unit.data?.manufacturer) {
    lines.push(`Manufacturer:${unit.data.manufacturer}`);
  }
  if (unit.data?.primaryfactory) {
    lines.push(`Primary Factory:${unit.data.primaryfactory}`);
  }
  
  // System manufacturers
  if (unit.data?.systemmanufacturer) {
    lines.push('System Manufacturer:');
    Object.entries(unit.data.systemmanufacturer).forEach(([system, manufacturer]) => {
      lines.push(`${system}:${manufacturer}`);
    });
  }
    ...
    if (line.includes(':') && !line.startsWith(' ')) {
      ...
      switch (key.toLowerCase()) {
        ...
        case 'history':
          ...
          unit.data![key.toLowerCase()] = '';
          break;
        case 'manufacturer':
          unit.data!.manufacturer = value;
          break;
        case 'primary factory':
          unit.data!.primaryfactory = value;
          break;
        default:
      ...
      if (currentSection === 'quirks' && unit.data!.quirks) {
        unit.data!.quirks.push(line);
      } else if (['overview', 'capabilities', 'deployment', 'history'].includes(currentSection)) {
        unit.data![currentSection] = (unit.data![currentSection] || '') + '\n' + line;
